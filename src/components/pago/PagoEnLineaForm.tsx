...
import { IconCheckCircle, IconCreditCard, IconShieldCheck, IconWhatsApp } from "@/components/ui/LadIcons";
...
export default function PagoEnLineaForm() {
  ...
  if (!PUBLIC_KEY) {
    return (
      <div className="rounded-3xl bg-white p-6 text-center shadow-glass sm:p-8">
        ...
        <p className="mt-2 text-sm text-lad-gray-mid">
  ...
  if (resultado) {
    ...
    return (
      <div className={`rounded-3xl p-6 text-center shadow-glass sm:p-8 ${esExito ? "bg-green-50" : "bg-red-50"}`}>
        ...
        <p className="mt-2 text-sm text-lad-gray-mid">{resultado.mensaje}</p>
        {resultado.idOperacion && (
          <p className="mt-3 text-xs font-bold uppercase tracking-wider text-lad-gray-mid">
  ...
  if (!confirmado) {
    return (
      <form
        ...
        className="rounded-3xl bg-white p-6 shadow-glass sm:p-8"
      >
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <input
              ...
              required
              className="w-full rounded-xl border border-lad-black/10 bg-white px-4 py-3.5 text-lad-black outline-none transition focus:border-lad-red"
            />
          </div>
          <div>
            <label className="mb-2 block text-[0.65rem] font-bold uppercase tracking-[0.18em] text-lad-gray-mid">
            ...
            <input
              ...
              inputMode="decimal"
              required
              className="w-full rounded-xl border border-lad-black/10 bg-white px-4 py-3.5 font-mono text-lg text-lad-black outline-none transition focus:border-lad-red"
            />
          </div>
          <div>
            <label className="mb-2 block text-[0.65rem] font-bold uppercase tracking-[0.18em] text-lad-gray-mid">
            ...
            <input
              ...
              className="w-full rounded-xl border border-lad-black/10 bg-white px-4 py-3.5 font-mono text-lad-black outline-none transition focus:border-lad-red"
            />
          </div>
        </div>
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-glass sm:p-8">
      <div className="mb-6 flex flex-col gap-3 border-b border-lad-black/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => setConfirmado(false)} className="text-xs font-bold uppercase tracking-wider text-lad-gray-mid underline hover:text-lad-red">
            Editar
          </button>
        </div>
      </div>

      <div className="mb-4 flex items-center gap-2 text-xs text-lad-gray-mid">
      ...
      {error && <p className="mb-4 rounded-r-xl border-l-4 border-lad-red bg-red-50 px-4 py-3 text-sm text-lad-red">{error}</p>}
      ...
      />
      {procesando && <p className="mt-3 text-center text-sm text-lad-gray-mid">Procesando tu pago…</p>}
    </div>
  );
}
